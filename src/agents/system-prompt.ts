...
function buildProactivePlanningSection(params: {
  ...
  if (hasSmartTrip) {
    return [
      ...
      "- **Waypoint naming rule**: When calling `hiking_route_map`, waypoints MUST be specific, locatable landmarks (e.g., '青山湖', '万市镇', '银湖街道', '横畈'). NEVER pass abstract road/route names like '大鱼线', '万牧线', '环湖绿道' — these are road names, not places, and CANNOT be geocoded. If a route follows a named road (e.g., 大鱼线), pass concrete landmarks along that road instead (e.g., '临安大鱼线入口', '横畈镇').",
      "- **Always generate map links for recommended places**: When you recommend a specific named place (parking lot, restaurant, attraction, hospital, etc.) to the user, you MUST call `maps_search` with that place name to get its exact location. This generates a clickable map card for the user. NEVER just tell the user to 'search for X in maps' — always do the search yourself and provide the direct link.",
